...
def df_from_hltv_player_stats(data, fantasyId):
  data_flat = []
  for team in data['moneyDraftData']['teams']:
    for player in team['players']:
      row = {}
      row['fantasyId'] = fantasyId
      row['leagueId'] = publicLeagueId
...
fantasyId = data['topMenuData']['fantasyId']['id']
publicLeagueName = data['topMenuData']['publicLeagueName']
publicLeagueId = data['topMenuData']['publicLeagueId']['id']

data = get_league_stats(EVENT_ID, publicLeagueId)

df = df_from_hltv_player_stats(data, EVENT_ID)
